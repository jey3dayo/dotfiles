...
def detect_relevant_skills(
    ...
    def add_skill(name: str, reason: str, confidence: float, triggers: List[str]) -> None:
    ...
    if language == "typescript" or "typescript" in desc_lower or "tsconfig" in desc_lower:
        add_skill(
            "typescript",
    ...
    if any(keyword in desc_lower for keyword in ["clean architecture", "usecase", "use case", "domain", "境界"]):
        add_skill(
            "clean-architecture",
            "レイヤリングと依存方向の制約をチェック",
            0.68,
    ...
    if any(keyword in desc_lower for keyword in ["impact", "dependency", "refactor", "breaking", "影響", "依存"]):
        add_skill(
            "semantic-analysis",
    ...
    if any(keyword in desc_lower for keyword in ["lint", "quality", "format", "readability", "可読性"]):
        add_skill(
    ...
    if any(keyword in desc_lower for keyword in ["doc", "docs", "markdown", "documentation", "readme"]):
        add_skill(
            "markdown-docs",
...
def _extract_project_metadata(context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for source in possible_sources:
        ...
        metadata["frameworks"].extend(
            _coerce_list(
                _pull(source, "frameworks")
                or _pull(source, "stack")
            )
        )
...
def classify_task(description: str, context: Optional[Dict[str, Any]] = None) -> TaskType:
    """
    ...
    investigation_keywords = [
        "why", "analyze", "investigate", "understand", "explore",
        "research", "find out", "discover", "what is", "how does"
    ]

    # Implementation keywords
    implementation_keywords = [
        "implement", "create", "build", "add", "develop",
        "construct", "make", "write", "code", "feature"
    ]

    # Error fixing keywords
    error_keywords = [
        "fix", "error", "bug", "issue", "problem",
        "broken", "failing", "eslint", "typescript error", "type error"
    ]

    # Code review keywords
    review_keywords = [
        "review", "quality", "best practice", "check",
        "evaluate", "assess", "audit", "verify"
    ]

    # Semantic analysis keywords
    semantic_keywords = [
        "impact", "dependency", "reference", "symbol",
        "api change", "breaking change", "refactor"
    ]

    # Exploration keywords
    exploration_keywords = [
        "where", "structure", "codebase", "architecture",
        "find files", "search for", "locate", "pattern"
    ]

    # CI diagnostics keywords
    ci_keywords = [
        "ci failure", "ci failed", "ci diagnose", "ci diagnosis",
        "ci失敗", "ci診断",
        "github actions", "workflow failure", "workflow failed",
        "failing checks", "check failed", "build error", "test failure"
    ]
...
def select_agent(task_type: TaskType, context: Optional[Dict[str, Any]] = None) -> AgentType:
    """
