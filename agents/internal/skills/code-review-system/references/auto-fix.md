...
PR レビューコメントを優先度別に分類し、`gh-fix-review` スキルと連携して自動修正を適用します。`.pr-review-config.json` による設定カスタマイズをサポートします。

## 実行フロー

### Step 1: コメント取得と分類

`gh-fix-review` スキルのコメント分類システムを使用:
...
## gh-fix-review スキルとの連携

`gh-fix-review` スキルは独自の設定システムを持つため、吸収ではなく参照で連携:
