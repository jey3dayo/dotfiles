...
Agent Skills の検証は3つのレイヤーで構成されています。
...
| 検証項目                                 | 説明                                                                     |
| ---------------------------------------- | ------------------------------------------------------------------------ |
| 選択スキルのバンドル存在確認             | `selectedSkills` の各スキルが `bundle/` 内にディレクトリとして存在するか |
| 選択スキルの SKILL.md 存在確認           | 各スキルディレクトリに `SKILL.md` が存在するか                           |
| バンドル内全ディレクトリの SKILL.md 確認 | バンドル内の全ディレクトリ（選択外含む）に `SKILL.md` があるか           |
| カタログ/選択スキル数の表示              | カタログ総数、選択数、未選択数を表示                                     |
| バンドルサイズの表示                     | バンドル全体のディスク使用量を表示                                       |
...
| 検証項目                  | 説明                                                             |
| ------------------------- | ---------------------------------------------------------------- |
| `.bundle-info` の存在確認 | バンドルのメタデータファイルが生成されているか                   |
| スキル数の一致確認        | `.bundle-info` 内の `count` と `selectedSkills` の数が一致するか |
...
| 検証項目                      | 説明                                                                    |
| ----------------------------- | ----------------------------------------------------------------------- |
| skills ディレクトリの存在確認 | `agents/internal/skills/` が存在するか                                  |
| symlink 禁止                  | skills ディレクトリ内に symlink が含まれていないか                      |
| SKILL.md 存在確認             | 各スキルディレクトリに `SKILL.md` または `skills/SKILL.md` が存在するか |
| スキル数 > 0                  | 少なくとも1つのスキルが存在するか                                       |
...
| 項目                                                    | 理由・対応                                                                                       |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `flake.nix` と `agent-skills-sources.nix` の URL 整合性 | Issue #131 で対応予定。Nix の flake inputs 静的制約により自動検証が困難                          |
| SKILL.md 以外の必須ファイル欠落                         | スキルの構造は SKILL.md のみが必須要件。他のファイルはスキル固有                                 |
| バンドルサイズの差分検知                                | バンドルサイズは表示のみ。閾値ベースのアラートは将来検討                                         |
| `catalogs/baseDir` パスの存在検証                       | Nix 評価時に `pathExists` で暗黙的に検証される（パスが存在しない場合はスキャンがスキップされる） |
