...
Finder / Dock / Spotlight から起動した GUI アプリは、通常の zsh 初期化ファイルや `~/.config/.env.local` を自動では読みません。GUI アプリへ渡す必要がある環境変数は、Home Manager の `launchd.agents.*` で `launchctl setenv` します。

- `codex-gui-path`: GUI アプリ向けの `PATH` を `launchctl` に登録する
- `codex-gui-env`: `scripts/setup-env.sh` で `~/.config/.env.local` を更新し、allowlist した値だけを `launchctl setenv` する

secret を GUI 環境へ渡す場合は、`.env.local` 全体をばらまかず、対象アプリが実際に参照するキーだけを明示的に allowlist します。たとえば Codex の `jina-reader` MCP は `JINA_API_KEY` を参照するため、`JINA_API_KEY` だけを `launchctl setenv` します。

allowlist を変更するときは、次の点を同じ差分で確認します。

- 対象キーを参照する GUI アプリと MCP / plugin / connector が明確であること
- `.env` / `.env.local` に存在するだけの unused secret を追加しないこと
- 不要になったキーは Home Manager の allowlist と runtime の `launchctl` 環境の両方から消すこと
- 変更後に `launchctl getenv <KEY>` で必要なキーだけが見えることを確認すること

```bash
# 例: Codex GUI へ JINA_API_KEY だけを渡す変更の確認
launchctl getenv JINA_API_KEY
launchctl getenv OPENAI_API_KEY
```

secret の展開先確認は `gitleaks` の代替ではありません。repo に残さない確認は `mise run ci:gitleaks`、GUI runtime に余計な値を出さない確認は `launchctl getenv` で分けて見ます。
