...
- tracked global package:
  ...
  - `~/.apm/catalog/rules/**`
  - ここが repo-tracked managed catalog tree であり、現在の source of truth
- manifest reference:
...
- transitional mirror:
  ...
  - `stage-catalog` が `catalog/` から再同期する
- not allowed:
...
`.config/agents/src/skills/`, `AGENTS.md`, `agents/`, `commands/`, `rules/` は transitional mirror として残します。
...
- `validate-catalog`: tracked catalog の正規化状態、manifest ref、transitional mirror drift を fail fast で検出する
