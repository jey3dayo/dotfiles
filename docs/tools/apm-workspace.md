...
この `.config` repo は bootstrap helper と APM CLI pin だけを持ち、日常の APM 操作は `~/.apm` から行います。
...
- 通常同期: `cd ~/.apm && mise run apply && mise run doctor`
...
external skill の upstream ref は `~/.apm/apm.yml` を正本として管理します。

- `.config` の Nix flake inputs には external skill source を持たない
- `~/.apm/apm.yml` と `~/.apm/apm.lock.yaml` に実際の install 状態を書き出す
- `mise run doctor` で `unpinned` と overlap を確認する

固定のスキル一覧はこの文書に持たず、必要なときは `~/.apm/apm.yml` を確認します。
...
external global skills を manifest に追加・更新する作業は `~/.apm` 側で行います。
...
`apm install -g <upstream-ref>` で `~/.apm/apm.yml` と `~/.apm/apm.lock.yaml` を更新し、managed catalog と競合しないことを `mise run doctor` で確認します。install 状態が崩れている時は `apm prune` を 1 回実行します。
...
- 通常導線: `cd ~/.apm && mise install && mise run apply && mise run doctor`
- 公式 installer:
