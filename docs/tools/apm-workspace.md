...
- tracked global package:
  ...
  - `~/.apm/catalog/agents/**`
  - `~/.apm/catalog/rules/**`
  - ここが repo-tracked managed catalog tree
- manifest reference:
...
- authoring source:
  ...
  - ここが managed catalog の source of truth
- not allowed:
...
`.config/agents/src/skills/`, `AGENTS.md`, `agents/`, `rules/` は fallback ではなく authoring lane として残します。
...
- `doctor`: workspace / targets / dependency 状態の確認。external の `unpinned` 件数、managed-vs-external の overlap 件数、catalog の `skills / agents / rules / instructions` 状態、target の `config / agents / rules / skills` presence も表示する
- `migrate-external`: enabled external skills を upstream ref として manifest へ記録
- `apply` / `update` / `register-catalog`: install 後に tracked `AGENTS.md` / `agents/` / `rules/` を runtime target に同期する
...
top-level managed `commands/` はまだ current model に含めません。`agents/src/` に authoritative な `commands/` tree が無いためです。
