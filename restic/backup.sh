...
HOMEBREW_BIN_DIR="${HOMEBREW_BIN_DIR:-/opt/homebrew/bin}"
...
if [[ -z "${RESTIC_REPOSITORY:-}" || -z "${RESTIC_PASSWORD:-}" ]]; then
  ...
  echo "Run via: mise run restic:backup (or dotenvx run -f .env.secrets -f .env -- restic/backup.sh)"
  exit 1
fi

# perman-aws-vault で AWS 一時クレデンシャル取得
echo "Fetching AWS credentials via perman-aws-vault..."
AWS_CREDS=$(PATH="${HOMEBREW_BIN_DIR}:${PATH}" perman-aws-vault print -p "$PROFILE")
if [[ -z "$AWS_CREDS" ]]; then
