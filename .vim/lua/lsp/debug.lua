...
function M.check_lsp_status()
  ...
  echo(eslint_pkg:is_installed() and "✓ installed" or "✗ not installed", 
       eslint_pkg:is_installed() and "DiagnosticOk" or "DiagnosticError")
  echo_newline()
  
  echo("• typescript-language-server: ", "Normal")
  echo(ts_ls_pkg:is_installed() and "✓ installed" or "✗ not installed",
  ...
  echo(global_format_disabled and "✗ OFF" or "✓ ON",
       global_format_disabled and "DiagnosticError" or "DiagnosticOk")
  echo_newline()
  
  echo("• Buffer formatting: ", "Normal")
  echo(buffer_format_disabled and "✗ OFF" or "✓ ON",
       buffer_format_disabled and "DiagnosticError" or "DiagnosticOk")
  echo_newline()
  ...
  echo(format_on_save and "configured" or "not configured", 
       format_on_save and "DiagnosticWarn" or "Normal")
  echo_newline()
...
function M.check_diagnostics()
  ...
  echo(diag_config.virtual_text and "Yes" or "No", 
       diag_config.virtual_text and "DiagnosticOk" or "DiagnosticError")
  echo_newline()
  ...
  for _, client in ipairs(clients) do
    ...
    local supports_diag = client.server_capabilities and 
  ...
  for _, client in ipairs(clients) do
    if client.server_capabilities.diagnosticProvider then
      vim.lsp.diagnostic.refresh(nil, { client_id = client.id })
    end
  end
...
end, { nargs = 1, complete = function() return { "eslint", "ts_ls" } end })
