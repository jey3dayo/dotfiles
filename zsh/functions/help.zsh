...
zsh-help() {
  ...
  keybinds | keys)
    ...
    echo "  ^gW / ^g^W     - git worktree manager (menu: Open/New/List/Remove)"
    echo "  ^gw / ^g^w     - git worktree open (direct selection)"
    echo "  ^gz / ^g^z     - fzf-git stash picker"
    echo "  ^g^f           - fzf-git file picker"
    echo "  ^g?            - fzf-git keybinding help"
    echo "  ^gx / ^g^x     - fzf kill process widget"
    echo "  ^R       - fzf history search"
    echo "  ^T       - fzf file finder"
    echo ""
    echo "Note: All ^g commands support both patterns:"
    echo "  - ^gX (Ctrl+g, release, X)"
    echo "  - ^g^X (Ctrl+g, hold Ctrl, X)"
    ;;
  aliases | abbr)
