...
var AutoUpdater_33836 = {
    ...
    call: function(response) {
	    ...
	    onload: function(xpr) {AutoUpdater_33836.compare(xpr,response);}
        });
    },
    enable: function() {
        GM_registerMenuCommand("Enable "+this.name+" updates", function() {
            GM_setValue('updated_33836', new Date().getTime()+'');
            AutoUpdater_33836.call(true)
        });
    },
    ...
    check: function() {
        if (GM_getValue('updated_33836', 0) == "off")
            this.enable();
        else {
            if (+this.time > (+GM_getValue('updated_33836', 0) + 1000*60*60*24*this.days)) {
                GM_setValue('updated_33836', this.time+'');
                this.call();
            }
