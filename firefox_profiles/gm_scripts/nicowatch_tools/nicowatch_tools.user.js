...
/*---------- HeatMeter ----------*/
	...
	var heatMeter = {
		...
		act : function(){
			if (GM_getValue("nicowatch_heatMeter", 1) == 1) {
				if (!$("#heatmeter").get(0)) {
					...
					doc1.innerHTML = '　<span style="font-size:14px;">Loading...</span>　　<a href="javascript:void(0);" style="font-size:10px;" id="heatmeter_bootlink">動かない場合はここをクリック</a>';
					doc.appendChild(doc1);
					doc1 = document.createElement("div");
		...
		calcurate : function(force){
			...
			for (i = 0; i < this.prm.widthsep; i++) {
				html += '<div class="h_meter" num="'+i+'" style="background-color:'+this.prm.color[Math.round(this.prm.chatNum[i]/this.prm.maxnum*0.15*100)]+';" title="コメント数：'+this.prm.chatNum[i]+'　　'+this.prm.titles[i]+'">&nbsp;</div>';
			}
...
//				console.debug( move_time + " : " + nowposition);
				...
				$('#heat_meter_body').css("background-position", (77+nowposition)+"px 18px");
			}, 200);
		}
	};
	GM_addStyle(<><![CDATA[
		...
		#heatmeter_top{
			height: 5px;
			width: 952px;
			background: url('data:image/gif;base64,R0lGODlhuAMFAIEAAGZmZvDw8JmZmf%2F%2F%2FywAAAAAuAMFAAAIqwAHCBwAoKDBgwgTKlzIsKHDhxAjSpxIsaLFixgzatzIsaPHjyBDihxJsqTJkyhTqlzJsiXLgQMLCghAs6bNmzhz6tzJs6fPn0CDCh1KtKjRo0iTKl3KtKnTp1CjSp1KtarVq1izat3KNauAgjFndh1LtqzZs2jTql3Ltq3bt3Djyp1L9KtAAHTz6t3Lt6%2Ffv4ADCx5MuHBNAATFGl7MuLHjx5AjS55Mue%2FXgAA7') no-repeat top left;
		}
		#heatmeter_container{
			width: 950px;
		...
		#heatmeter_bottom{
			...
			width: 952px;
			margin-bottom: 5px;
			background: url('data:image/gif;base64,R0lGODlhuAMFAIEAAGZmZvDw8JmZmf%2F%2F%2FywAAAAAuAMFAAAIrQABCAhAsKDBgwgTKlzIsKHDhxAjSpxIsaLFixgzatzIsaPHjyBDihxJsqTJkyhTqlzJsiVBAQAGAHBJs6bNmzhz6tzJs6fPn0CDCh1KtGBMmQOLKl3KtKnTp1CjSp1KtSpRmAOyAhBotavXr2DDih1LtqzZojCPZtW6ta3bt3Djyp1Lt67du3jz6t3Lt6%2Ffv4ADCx5MuLDhw4gTK17MuLHjx5AjS55MefLaAQEBADs%3D') no-repeat bottom left;
		}
	]]></>);
