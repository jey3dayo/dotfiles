...
var NicoClip = function() {
  this.hatenaBookmarkTag = 'nicovideo'; // はてなブックマークで付けるタグ
...
NicoClip.prototype.showPlaylistData = function(videos) {
  ...
  closeLink.appendChild(d.createTextNode('×'));
...
NicoClip.prototype.parsePage_playlistFeed = function(resp, url, doAppend) {
  ...
  for (var i = 0; i < entriesNum; i++) {
    ...
    var image;
    if (link.indexOf('http://www.nicovideo.jp/watch/') != -1) {
      image = ['http://tn-skr.smilevideo.jp/smile?i=', link.replace(/http:\/\/www.nicovideo.jp\/watch\/../,'')].join('');
    }
    var video = {
      'title': title,
      'link' : link,
...
NicoClip.prototype.getHatenaUsername = function(callback, args) {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(resp) {
      ...
      var startTag = '<a href="/my">';
      var startPos = datastr.indexOf(startTag);
      if (startPos != -1) {
