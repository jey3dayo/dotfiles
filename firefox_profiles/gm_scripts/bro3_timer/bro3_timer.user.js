...
function addOpenLinkHtml() {
	...
	openLink.addEventListener("click", function() {openTimer()}, true);
...
//	timerContainer.style.opacity = 0.8;
	...
	versionLink.href = "http://blog.livedoor.jp/froo/archives/51423697.html";
...
function getMyVillage() {
	...
	var elems = document.evaluate('//*[@id="sidebar"]//div[2]/div[2]/ul',
		document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	for (var i = 0; i < elems.snapshotItem(0).childNodes.length; i++) {
		var item = elems.snapshotItem(0).childNodes[i];
		if (item.nodeName.toLowerCase() == "li" && item.className == "on") {
			var myVilElem = getChildElement(item, 0);
			ret[IDX_XY] = myVilElem.title.replace(/^.+(\(-?[0-9]+\,-?[0-9]+\))$/, "$1");
			ret[IDX_BASE_NAME] = trim(myVilElem.innerHTML);
			return ret;
		}
	}
