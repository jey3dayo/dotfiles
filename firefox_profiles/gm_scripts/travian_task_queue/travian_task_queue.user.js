// ==UserScript==
// @author      Risi (edited for 3.5 by Nevam) (edited by Pimp Trizkit for the break of 10/29/09)
 ...
 *********************/
if (init) {
    var sCurrentVersion = "1.8.5";  //Version number with which we need to run the update fu
	...
	* -> Please post all translations here: http://userscripts.org/topics/27738
	...
	case "no":  //by Pimp Trizkit (in Norwenglish)
		...
		aLangStrings = ["Build later", "Upgrade later", "Attack later", "Research later", "Schedule this task for later.", "We started builing ", " was attempted with unknown result.", "Nivå", " cannot be built.", " cannot be upgraded.", "The task was scheduled.", "Produksjon:",         "We can't schedule this task right now.", "Task scheduling is not available!", "Scheduled Tasks", "Delete", "Send later", "No troops were selected.", "Your troops were sent to", "Your troops could not be sent to", "Support", "Attack", "Raid", "Catapults will aim at", "random", "at", "or after", "seconds", "minutes", "hours", "days", "Spy for resources and troops", "Spy for troops and defenses", "away", "The attack cannot be scheduled because no destination was specified.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ", "Task History", "flush history", "We started researching ", " cannot be researched.", "Enhance later", "Spy", "train later", "troops.", "Train", "We started training ", " cannot be trained.", "Party Later", " but not today.", "We started to "];
...
function createBuildLinks() {
	...
	for (var i = 0; i < xpathBuildingNames.snapshotLength; ++i) {
		...
		if ( sBuildingName.indexOf("span") != -1 ) sBuildingName = sBuildingName.split("<")[0] + sBuildingName.split("<")[1].split(">")[1];
...
function createAttackLinks() {
	...
	if( heroBox[0].firstChild == null )   //no hero textbox - make one
		heroBox[0].innerHTML = "<img class='unit uhero' src='img/x.gif' title='' onclick='document.snd.t11.value=\"\"; return false;' alt='' /><input type='text' class='text' name='t11' value='' maxlength='1' /><a href='#' onclick='document.snd.t11.value=1; return false;'>(1)</a>&nbsp;<b>(" +aLangStrings[33]+ ")</b>";
...
function createPartyLinks() {
	...
	var bn = document.getElementsByTagName("h1")[0].innerHTML.split("<")[0].slice(0,-1);

	if(bn == aLangBuildings[24]) {
		...
		for(var i = 0; (i < xpathStart.length); i++) {
			...
			var sBuildingId = aLangBuildings.indexOf(sBuildingName);
			var thisStart = xpathStart.item(i);
...
function createResearchLinks() {
	...
	var Imgs = document.getElementsByClassName("unit");
	var Cells = document.getElementsByClassName("act");
	...
	for(var i = 0; (i < Imgs.length) && (i < Cells.length); i++) {
		var thisImg = Imgs.item(i).getAttribute('class');
		var thisCell = Cells.item(i);
